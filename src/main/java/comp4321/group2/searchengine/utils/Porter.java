...
public class Porter {
    ...
    private boolean hasSuffix(String word, String suffix, NewString stem) {
        ...
        if (suffix.length() > 1) if (word.charAt(word.length() - 2) != suffix.charAt(suffix.length() - 2)) return false;

        stem.str = "";

        for (int i = 0; i < word.length() - suffix.length(); i++) stem.str += word.charAt(i);
        tmp = stem.str;
    ...
    private int measure(String stem) {
        ...
        while (i < length) {
            for (; i < length; i++) {
                if (i > 0) {
                    if (vowel(stem.charAt(i), stem.charAt(i - 1))) break;
                } else {
                    if (vowel(stem.charAt(i), 'a')) break;
                }
            }
    ...
    private boolean containsVowel(String word) {
        for (int i = 0; i < word.length(); i++) if (i > 0) {
            if (vowel(word.charAt(i), word.charAt(i - 1))) return true;
        } else {
            if (vowel(word.charAt(0), 'a')) return true;
        }
    ...
    private boolean cvc(String str) {
        ...
        if (
            (!vowel(str.charAt(length - 1), str.charAt(length - 2))) &&
    ...
    private String step5(String str) {
        if (str.charAt(str.length() - 1) == 'e') {
            if (measure(str) > 1) {/* measure(str)==measure(stem) if ends in vowel */
                ...
                String stem = "";
                for (int i = 0; i < str.length() - 1; i++) stem += str.charAt(i);
        ...
        if (str.length() == 1) return str;
        if ((str.charAt(str.length() - 1) == 'l') && (str.charAt(str.length() - 2) == 'l') && (measure(str) > 1)) if (
            measure(str) > 1
